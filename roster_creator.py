...
class Player:
    def __init__(self, firstName="", lastName="", default_rating=-1) -> None:
        ...
        self.teamID = ""
        self.teamNum = 0  # TEST
        # random generated as backup if we can't find the real year
        self.draftSeason = random.randint(2012, 2020)
        ...
        self.growthType = [
            ...
            -1400,
            -1400
        ]  # PGM specific - development trait

        # contract
        self.salary = 10534000  # game will set automatically
        ...
        # TODO: not present in maddenratings.com
        self.intelligence = random.randint(20, 100)

        # personality attributes
        # TODO: not present in maddenratings.com
        self.discipline = random.randint(10, 100)
...
def generatePlayerFromURL(player_name, player_url):
    ...
    new_player = Player(
        firstName=player_name_split[0], lastName=player_name_split[1])
    ...
    for attr in background_attrs:
        ...
        if "Pro: " in attr_split:
            ...
            years_pro = int(
                ''.join(c for c in attr_split[years_pro_ind + 1] if c.isdigit()))
        ...
        # set position and age
            ...
            new_player.age = int(
                ''.join(c for c in attr_split[years_ind - 1] if c.isdigit()))
    # set rating
    ...
    for attribute in attributes:
        ...
        if attr_name in maddenToPGM:
            ...
            if pgm_attr_name == "routeRun":
                setattr(new_player, pgm_attr_name, int(
                    getattr(new_player, pgm_attr_name) + 0.33 * attr_val))
                ...
                setattr(new_player, pgm_attr_name, int(
                    getattr(new_player, pgm_attr_name) + 0.5 * attr_val))
            # general statement
            else:
                setattr(new_player, pgm_attr_name, attr_val)
...
def createUpdatedRoster():
    ...
    team_pages = [elem["href"] for elem in main_page_soup.select(
        'a.sidebar-link.sidebar-link-open')]
    temp_count = 1
    final_roster = []
    ...
    for team_page in team_pages:
        ...
        team_page_soup = BeautifulSoup(
            requests.get(team_page).text, "html.parser")

        # iterate through each player, create Player model for player and append to final roster
        # use random ratings for free agents
        if team_name == "Free Agency":
            players = tqdm([(elem.text, elem['href'])
                           for elem in team_page_soup.select("span.entry-font a")])
        else:
            players = tqdm([(elem['title'], elem['href'])
                           for elem in team_page_soup.select("span.entry-font a")])
        ...
        for player_name, player_url in players:
            ...
            if team_name == "Free Agency":
                player_name_split = player_name.split(" ", 1)
                player_model = Player(
                    firstName=player_name_split[0], lastName=player_name_split[1], default_rating=random.randint(60, 80))
